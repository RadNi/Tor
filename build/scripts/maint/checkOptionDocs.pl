...
open(F, "/root/git/Tor/Tor/build/src/or/tor --list-torrc-options |") or die;
while (<F>) {
...
loadTorrc("/root/git/Tor/Tor/build/../src/config/torrc.sample.in", \%torrcSampleOptions);

# Try to figure out what's in the man page.

my $considerNextLine = 0;
open(F, "/root/git/Tor/Tor/build/../doc/tor.1.txt") or die;
while (<F>) {
